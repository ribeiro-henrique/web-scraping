...
export class MedCofService {
  async getRmais(): Promise<[object]> {
    ...
    const semiExtensivo = await page.evaluate(() => {
      ...
      const valorCheio =
        document.getElementsByClassName('mt-2 text-slate-50')[1];
      const quantidade =
        document.getElementsByClassName('mt-2 text-slate-50')[0];
    ...
    const semiExtensivoElite = await page.evaluate(() => {
      ...
      const valorCheio =
        document.getElementsByClassName('mt-2 text-slate-50')[3];
      const quantidade =
        document.getElementsByClassName('mt-2 text-slate-50')[2];
    ...
    const semiExtensivoPremium = await page.evaluate(() => {
      ...
      const valorCheio =
        document.getElementsByClassName('mt-2 text-slate-50')[5];
      const quantidade =
        document.getElementsByClassName('mt-2 text-slate-50')[4];
  ...
  async getRUm(): Promise<[object]> {
    const browser = await puppeteer.launch();
    const page = await browser.newPage();

    // Navigate the page to a URL
    await page.goto(
      'https://semiextensivo.grupomedcof.com.br/r1-acesso-direto#prices',
    );
